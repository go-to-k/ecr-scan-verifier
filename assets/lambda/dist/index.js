...
`)};var S=require("@aws-sdk/client-cloudwatch-logs"),P=new S.CloudWatchLogsClient,v=async(t,o,n,e)=>{let s=e.replace(/:/g,",").replace(/\//g,"_"),i=`${s}/findings`,a=`${s}/summary`,c=new Date().getTime();return await A(n.logGroupName,i,c,t),await A(n.logGroupName,a,c,o),console.log(`Scan logs output to the log group: ${n.logGroupName}
  findings stream: ${i}
  summary stream: ${a}`),{type:"cloudwatch",logGroupName:n.logGroupName,findingsLogStreamName:i,summaryLogStreamName:a}},G=1048576,Q=t=>{let n=new TextEncoder().encode(t);if(n.length<=G)return[t];let e=[],s=0,a=G-20;for(;s<n.length;){let c=n.slice(s,s+a),r=new TextDecoder("utf-8",{fatal:!1});e.push(r.decode(c)),s+=a}return e},A=async(t,o,n,e)=>{try{await P.send(new S.CreateLogStreamCommand({logGroupName:t,logStreamName:o}))}catch(u){if(u instanceof S.ResourceAlreadyExistsException)console.log(`Log stream ${o} already exists in log group ${t}.`);else throw u}let s=Q(e),i=s.length;i>1&&console.log(`Message size exceeds 1 MB limit. Splitting into ${i} chunks.`);let a=s.map((u,m)=>({timestamp:n+m,message:i>1?`[part ${m+1}/${i}] ${u}`:u})),c={logGroupName:t,logStreamName:o,logEvents:a},r=new S.PutLogEventsCommand(c);await P.send(r)};var C=require("@aws-sdk/client-s3");var $=new C.S3Client,B=async(t,o,n,e,s)=>{let i=new Date().toISOString(),a=e.replace(/:/g,"/").replace(/\//g,"_"),r=`${n.prefix?n.prefix.endsWith("/")?n.prefix:`${n.prefix}/`:""}${a}/${i}`,u=`${r}/findings.json`,m=`${r}/summary.txt`,p=[$.send(new C.PutObjectCommand({Bucket:n.bucketName,Key:u,Body:t,ContentType:"application/json"})),$.send(new C.PutObjectCommand({Bucket:n.bucketName,Key:m,Body:o,ContentType:"text/plain"}))],g;if(s){let d=s.format==="SPDX_2_3"?"spdx.json":"cyclonedx.json";g=`${r}/sbom.${d}`,p.push($.send(new C.PutObjectCommand({Bucket:n.bucketName,Key:g,Body:s.content,ContentType:"application/json"})))}return await Promise.all(p),console.log(g?`Scan logs and SBOM output to S3:
...
${o}`,u={default:r,email:r,https:JSON.stringify(c)};try{await Z.send(new w.PublishCommand({TopicArn:t,Message:JSON.stringify(u),MessageStructure:"json"})),console.log(`Vulnerability notification sent to SNS topic: ${t}`)}catch(m){console.error(`Failed to send vulnerability notification to SNS: ${m}`)}};var y=require("@aws-sdk/client-cloudformation"),nn=new y.CloudFormationClient,_=async t=>{let o=new y.DescribeStacksCommand({StackName:t}),n=await nn.send(o);if(n.Stacks&&n.Stacks.length>0){let e=n.Stacks[0].StackStatus;return e===y.ResourceStatus.ROLLBACK_IN_PROGRESS||e===y.ResourceStatus.UPDATE_ROLLBACK_IN_PROGRESS}throw new Error(`Stack not found or no stacks returned from DescribeStacks command, stackId: ${t}`)};var f=require("@aws-sdk/client-inspector2"),h=require("@aws-sdk/client-s3"),W=new f.Inspector2Client,en=new h.S3Client,tn=t=>new Promise(o=>setTimeout(o,t)),M=async(t,o,n,e,s)=>{let i=n==="SPDX_2_3"?f.SbomReportFormat.SPDX_2_3:f.SbomReportFormat.CYCLONEDX_1_4;console.log(`Starting SBOM export for ${t} with format ${n}...`);let a={ecrRepositoryName:[{comparison:"EQUALS",value:t}],...o?{ecrImageTags:[{comparison:"EQUALS",value:o}]}:{}},r=(await W.send(new f.CreateSbomExportCommand({reportFormat:i,s3Destination:{bucketName:e,keyPrefix:`sbom-exports/${t}`,kmsKeyArn:s},resourceFilterCriteria:a}))).reportId;if(!r)throw new Error("CreateSbomExport did not return a reportId.");console.log(`SBOM export started with reportId: ${r}`);let u=60,m=5;for(let p=0;p<u;p++){let g=await W.send(new f.GetSbomExportCommand({reportId:r})),d=g.status;if(console.log(`SBOM export status: ${d} (attempt ${p+1}/${u})`),d==="SUCCEEDED"){let l=g.s3Destination?.keyPrefix,I=g.s3Destination?.bucketName;if(!I||!l)throw new Error("SBOM export succeeded but S3 destination is missing.");return{sbomContent:await sn(I,l),format:n}}if(d==="FAILED"){let l=g.filterCriteria;throw new Error(`SBOM export failed. Filter criteria: ${JSON.stringify(l)}`)}if(d==="CANCELLED")throw new Error("SBOM export was cancelled.");await tn(m*1e3)}throw new Error(`SBOM export timed out after ${u*m} seconds.`)},sn=async(t,o)=>await(await en.send(new h.GetObjectCommand({Bucket:t,Key:o}))).Body?.transformToString()??"";var U=async function(t){let o=t.RequestType,n=t.ResourceProperties;if(!n.addr||!n.repositoryName)throw new Error("addr and repositoryName are required.");let e={PhysicalResourceId:n.addr,Data:{}};if(o!=="Create"&&o!=="Update")return e;let s=5,i=60,a=`${n.repositoryName}:${n.imageTag}`,c;n.startScan==="true"?c=await x(n.repositoryName,n.imageTag,n.scanType,s,i):c=await O(n.repositoryName,n.imageTag,n.scanType,s,i);let r=R(c,n.severity,n.ignoreFindings),u;if(n.sbom)if(n.scanType==="ENHANCED")try{let l=await M(n.repositoryName,n.imageTag,n.sbom.format,n.sbom.bucketName,n.sbom.kmsKeyArn);u={content:l.sbomContent,format:l.format}}catch(l){console.error(`SBOM export failed (non-fatal): ${l}`)}else console.log("SBOM export is only available with Enhanced scanning. Skipping SBOM generation.");let m=JSON.stringify(c.rawResponse.imageScanFindings,null,2),p=T(c,r,n.repositoryName,n.imageTag),g=await on(m,p,a,n.output,n.defaultLogGroupName,u);if(!r.hasVulnerabilities)return e;let d=`ECR Image Scan found vulnerabilities.
Image: ${a}
Scan Type: ${c.scanType}
Findings: ${r.summary}
See scan logs for details.`;if(n.vulnsTopicArn&&await D(n.vulnsTopicArn,d,a,g),n.failOnVulnerability==="false")return e;if(n.suppressErrorOnRollback==="true"&&await _(t.StackId))return console.log(`Vulnerabilities detected, but suppressing errors during rollback (suppressErrorOnRollback=true).
${d}`),e;throw new Error(d)},on=async(t,o,n,e,s,i)=>{switch(e?.type){case"cloudWatchLogs":return await v(t,o,e,n);case"s3":return await B(t,o,e,n,i);default:return console.log(`summary:
